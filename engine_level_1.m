function [ p ] = engine_level_1( Dtype, Ttype, Ntype )
%Funkcja stworzy zbiór sprawnoœci, sprê¿ów i maks. temperatur dla charakterystycznych
%elementów silnika poziomu "1" (najgorszego) na podstawie wpisanego typu wlotu i wylotu
%(szczegó³y na str. 107 w Mattingly - Aircraft Engine Design Second
%Edition.

%Wlot (pi)
switch Dtype
    case 1
        p(1) = 0.9; %samolot poddŸwiêkowy z silnikami w gondolach
    case 2
        p(1) = 0.88; %samolot poddŸwiêkowy z silnikiem w kad³ubie
    case 3
        p(1) = 0.85; %samolot naddŸwiêkowy z silnikiem w kad³ubie
    otherwise
        error('Poda³eœ z³y typ wlotu!')
end

p(2) = 0.8; %Sprê¿arka (eta)
p(3) = 0.78; %Wentylator (eta)
p(4) = 0.9; %Komora spalania (pi)
p(5) = 0.88; %Komora spalania (eta)
p(6) = 0.8; %Turbina (eta)
p(7) = 0.9; %Dopalacz (pi)
p(8) = 0.85; %Dopalacz (eta)

%Dysza (pi)
switch Ntype
    case 1
        p(9) = 0.95; %dysza zbie¿na o sta³ym przekroju
    case 2
        p(9) = 0.93; %dysza zbie¿na o zmiennym przekroju
    case 3
        p(9) = 0.9; %dysza zbie¿no-rozbie¿na o zmiennym przkeroju
    otherwise
         error('Poda³eœ z³y typ wylotu!')
end

p(10) = 1110; %Tt4 (K)
p(11) = 1390; %Tt7 (K)
p(12) = 0.99; %Sprawnoœæ mechaniczna sprê¿arka - turbina

end