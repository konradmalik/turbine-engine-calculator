function varargout = iteration_altitude_postproc_no_ab(varargin)
% ITERATION_ALTITUDE_POSTPROC_NO_AB MATLAB code for iteration_altitude_postproc_no_ab.fig
%      ITERATION_ALTITUDE_POSTPROC_NO_AB, by itself, creates a new ITERATION_ALTITUDE_POSTPROC_NO_AB or raises the existing
%      singleton*.
%
%      H = ITERATION_ALTITUDE_POSTPROC_NO_AB returns the handle to a new ITERATION_ALTITUDE_POSTPROC_NO_AB or the handle to
%      the existing singleton*.
%
%      ITERATION_ALTITUDE_POSTPROC_NO_AB('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ITERATION_ALTITUDE_POSTPROC_NO_AB.M with the given input arguments.
%
%      ITERATION_ALTITUDE_POSTPROC_NO_AB('Property','Value',...) creates a new ITERATION_ALTITUDE_POSTPROC_NO_AB or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before iteration_altitude_postproc_no_ab_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to iteration_altitude_postproc_no_ab_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help iteration_altitude_postproc_no_ab

% Last Modified by GUIDE v2.5 13-Jun-2015 21:50:03

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @iteration_altitude_postproc_no_ab_OpeningFcn, ...
                   'gui_OutputFcn',  @iteration_altitude_postproc_no_ab_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before iteration_altitude_postproc_no_ab is made visible.
function iteration_altitude_postproc_no_ab_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to iteration_altitude_postproc_no_ab (see VARARGIN)

% Choose default command line output for iteration_altitude_postproc_no_ab
...
handles.headers = {'Altitude [m]', 'Specific impulse (internal) [m/s]', 'Specific impulse (external) [m/s]', 'Specific impulse [m/s]', 'Fuel flow [kg/s]', 'Specific fuel consumption [kg/(N*s)]','Pt9/P9', 'P0/P9', 'V9/V0', 'Thermal efficiency', 'Propulsive efficiency', 'General efficiency'};

% Update handles structure
guidata(hObject, handles);
% UIWAIT makes iteration_altitude_postproc_no_ab wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = iteration_altitude_postproc_no_ab_OutputFcn(hObject, eventdata, handles) 
...
save_engine_given_parameters_txt( handles.given_parameters_name, M0, nan, mdot0, Tt4, nan, piF, FtoLPC, piC, BR, vc, cool1, cool2 )
...
elseif strcmpi(contents{get(hObject,'Value')}, 'Given parameters')
    ...
    figure;
    uitable;
    given_param = [M0, nan, mdot0, Tt4, nan, piF, FtoLPC, piC, BR, vc, cool1, cool2];
...
close(iteration_altitude_postproc_no_ab)
run('start')
