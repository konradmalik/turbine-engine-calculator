function varargout = postproc_no_ab(varargin)
% POSTPROC_NO_AB MATLAB code for postproc_no_ab.fig
%      POSTPROC_NO_AB, by itself, creates a new POSTPROC_NO_AB or raises the existing
%      singleton*.
%
%      H = POSTPROC_NO_AB returns the handle to a new POSTPROC_NO_AB or the handle to
%      the existing singleton*.
%
%      POSTPROC_NO_AB('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in POSTPROC_NO_AB.M with the given input arguments.
%
%      POSTPROC_NO_AB('Property','Value',...) creates a new POSTPROC_NO_AB or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before postproc_no_ab_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to postproc_no_ab_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help postproc_no_ab

% Last Modified by GUIDE v2.5 28-Jun-2015 13:12:34

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @postproc_no_ab_OpeningFcn, ...
                   'gui_OutputFcn',  @postproc_no_ab_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before postproc_no_ab is made visible.
function postproc_no_ab_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to postproc_no_ab (see VARARGIN)

% Choose default command line output for postproc_no_ab
...
% Update handles structure
guidata(hObject, handles);
% UIWAIT makes postproc_no_ab wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = postproc_no_ab_OutputFcn(hObject, eventdata, handles) 
...
save_engine_given_parameters_txt( handles.given_parameters_name, M0, height, mdot0, Tt4, nan, piF, FtoLPC, piC, BR, vc, cool1, cool2 )
...
kjint = evalin('base','kjint');
kjext = evalin('base','kjext');
kj = evalin('base','kj');
Tr = evalin('base','Tr');
bj = evalin('base','bj');
...
eta = evalin('base','eta');
save_engine_calculated_parameters_txt( handles.main_parameters_name, kjint, kjext, kj, Tr, bj, etat, etap, eta )
...
elseif strcmpi(contents{get(hObject,'Value')}, 'Given parameters')
    ...
    figure;
    uitable;
    given_param = [M0, height, mdot0, Tt4, nan, piF, FtoLPC, piC, BR, vc, cool1, cool2];
...
elseif strcmpi(contents{get(hObject,'Value')}, 'Main engine parameters')
    ...
    figure;
    uitable;
    main_param = [kjint, kjext, kj, Tr, bj, etat, etap, eta];
    column_names = {'Specific impulse (internal) [m/s]' 'Specific impulse (external) [m/s]', 'Specific impulse [m/s]', 'Reduced Thrust [N]', 'Specific fuel consumption [kg/(N*s)]', 'Thermal efficiency', 'Propulsive efficiency', 'General efficiency'};
...
% --- Executes during object creation, after setting all properties.
function graph_maker_CreateFcn(hObject, eventdata, handles)
% hObject    handle to graph_maker (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
